...
The official public URL surface is only the ingress catalog declared in `k8s/charts/haac-stack/config-templates/values.yaml.template` and rendered into `k8s/charts/haac-stack/values.yaml`. `verify-web`, Homepage, and the generated `HTTPRoute` set all derive from that same source of truth. Official UI routes are protected by Authelia forward-auth by default unless a route is explicitly marked public. Hosts outside that catalog still fall through the wildcard tunnel and correctly return a Traefik `404`.
