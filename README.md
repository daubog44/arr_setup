...
The official public URL surface is only the ingress catalog declared in `k8s/charts/haac-stack/config-templates/values.yaml.template` and rendered into `k8s/charts/haac-stack/values.yaml`. `verify-web`, Homepage, and the generated `HTTPRoute` set all derive from that same source of truth. Every published route must now declare an explicit `auth_strategy` (`public`, `edge_forward_auth`, `native_oidc`, or `app_native`), and rendering fails closed if a route omits that field. The current Cloudflare tunnel still exposes a wildcard catch-all as a transitional publication model; hosts outside the official catalog are not product URLs and should be treated as unsupported until the explicit-publication change lands.
