...
The official public URL surface is only the ingress catalog declared in `k8s/charts/haac-stack/config-templates/values.yaml.template` and rendered into `k8s/charts/haac-stack/values.yaml`. `verify-web` and Homepage both derive from that same source of truth. Hosts outside that catalog, such as `falco.<domain>` or `litmus.<domain>`, are not published by default; because Cloudflare tunnels the wildcard domain to Traefik, those unsupported hosts correctly return a Traefik `404`.
