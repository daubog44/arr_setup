...
$KUBECTL create secret generic protonvpn-key -n media \
  ...
  $KUBESEAL --format=yaml --cert="$PUB_CERT" --scope cluster-wide > "$CHART_DIR/secrets/protonvpn-sealed-secret.yaml"
...
$KUBECTL create secret generic cloudflare-tunnel-token -n cloudflared \
  ...
  --dry-run=client -o yaml | \
  $KUBESEAL --format=yaml --cert="$PUB_CERT" --scope cluster-wide > "$CHART_DIR/secrets/cloudflared-sealed-secret.yaml"
...
$KUBECTL create secret generic authelia-config-files -n mgmt \
  ...
  --dry-run=client -o yaml | \
  $KUBESEAL --format=yaml --cert="$PUB_CERT" --scope cluster-wide > "$CHART_DIR/secrets/authelia-sealed-secret.yaml"
...
$KUBECTL create secret generic argocd-notifications-custom-secret -n argocd \
  ...
  --dry-run=client -o yaml | \
  $KUBESEAL --format=yaml --cert="$PUB_CERT" --scope cluster-wide > "$CHART_DIR/secrets/argocd-notifications-sealed-secret.yaml"
...
$KUBECTL create secret generic argocd-sso-secret -n argocd \
  ...
  --dry-run=client -o yaml | \
  $KUBESEAL --format=yaml --cert="$PUB_CERT" --scope cluster-wide > "$CHART_DIR/secrets/argocd-sso-sealed-secret.yaml"

echo "5.6 Creazione SealedSecret per ArgoCD OIDC..."
$KUBECTL create secret generic argocd-oidc-secret -n argocd \
  --from-literal=client_secret="${ARGOCD_OIDC_SECRET}" \
  --dry-run=client -o yaml | \
  $KUBESEAL --format=yaml --cert="$PUB_CERT" --scope cluster-wide > "$CHART_DIR/secrets/argocd-oidc-sealed-secret.yaml"

echo "5.7 Creazione SealedSecret per Grafana OIDC..."
$KUBECTL create secret generic grafana-oidc-secret -n monitoring \
  --from-literal=clientSecret="${GRAFANA_OIDC_SECRET}" \
  --dry-run=client -o yaml | \
  $KUBESEAL --format=yaml --cert="$PUB_CERT" --scope cluster-wide > "$CHART_DIR/secrets/grafana-oidc-sealed-secret.yaml"
...
if [ -f "$SSH_KEY" ]; then
  ...
  $KUBECTL create secret generic haac-ssh-key -n mgmt \
    ...
    --dry-run=client -o yaml | \
    $KUBESEAL --format=yaml --cert="$PUB_CERT" --scope cluster-wide > "$CHART_DIR/secrets/haac-ssh-sealed-secret.yaml"
else
...
$KUBECTL create secret generic semaphore-db-secret -n mgmt \
  ...
  --dry-run=client -o yaml | \
  $KUBESEAL --format=yaml --cert="$PUB_CERT" --scope cluster-wide > "$CHART_DIR/secrets/semaphore-sealed-secret.yaml"
