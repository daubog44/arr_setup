...
variable "lxc_unprivileged" {
  description = "Whether the Proxmox LXC containers should remain unprivileged. This should stay true unless you have a very specific host-level compatibility issue."
  type        = bool
  default     = true
}

variable "lxc_nesting" {
  description = "Whether nested container features should be enabled inside the LXC nodes for K3s/containerd workloads."
  type        = bool
  default     = true
}
...
variable "python_executable" {
  description = "Python executable used by local OpenTofu external data sources"
  type        = string
  default     = "python3"
}
