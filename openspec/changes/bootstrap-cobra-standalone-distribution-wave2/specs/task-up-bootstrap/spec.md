...
- **WHEN** an operator wants to create or reconcile the homelab from an initialized HaaC workspace
- **THEN** the supported entrypoint MUST be `haac up`
- **AND** compatibility shims such as `task up`, `.\haac.ps1 up`, and `sh ./haac.sh up` MAY invoke the same pipeline

#### Scenario: Shared orchestration contract
- **WHEN** the operator runs `haac up`, `task up`, `.\haac.ps1 up`, or `sh ./haac.sh up`
