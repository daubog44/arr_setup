...
The overlay must not depend on a remote Kustomize Git URL at runtime. ArgoCD currently times out fetching the remote base during manifest generation, which leaves the self-managed app at `sync=Unknown`. The fix is to vendor the pinned upstream manifests into the repo and point the overlay at the local vendored subtree instead.
