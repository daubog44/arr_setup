...
- Browser verification later in the same round reached `home.nucleoautogenerativo.it` and also received `502 Bad gateway` from Cloudflare.
...
- Live cluster inspection for Homepage showed `discovery.k8s.io/v1 EndpointSlice/homepage-*` rendered with `ready: false` on the master while the deployment still used the one-second default HTTP probe timeout for `/api/healthcheck`.
...
- Both Headlamp and Homepage leave their HTTP probes on Kubernetes defaults for `timeoutSeconds`, which means a one-second timeout for their health endpoints.
...
- raise the HTTP probe timeout budget so transient latency does not flap Headlamp or Homepage out of the EndpointSlice
...
- explicit Playwright CLI navigation for Headlamp and Homepage after reconcile
