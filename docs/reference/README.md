...
Use these guides when `README.md` is no longer enough and you need the detailed behavior behind `haac up`, the media automation flow, or the layered security posture.

## Guides

- [operator-bootstrap.md](./operator-bootstrap.md): what `haac up` does phase by phase, how `.env` maps into generated outputs, and which rerun paths are supported.
...
- Read `operator-bootstrap.md` before touching `haac up`, `haac down`, or the generated GitOps artifacts.
